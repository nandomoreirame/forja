...
describe("TerminalContextMenu", () => {
  beforeEach(() => {
    mockSplitActiveTabset.mockClear();
  ...
  it("calls splitActiveTabset with vertical when Split Vertical is clicked", async () => {
    const user = userEvent.setup();
    ...
    expect(mockSplitActiveTabset).toHaveBeenCalledWith("vertical");
  });

  it("calls splitActiveTabset with horizontal when Split Horizontal is clicked", async () => {
    const user = userEvent.setup();
    ...
    expect(mockSplitActiveTabset).toHaveBeenCalledWith("horizontal");
  });
  ...
  it("does not show Close Split option (removed in tiling layout)", async () => {
    const user = userEvent.setup();
  ...
  it("split options are always enabled (flexlayout supports unlimited splits)", async () => {
    const user = userEvent.setup();
    ...
    expect(splitVertical).not.toHaveAttribute("data-disabled");
    expect(splitHorizontal).not.toHaveAttribute("data-disabled");
  });
});
